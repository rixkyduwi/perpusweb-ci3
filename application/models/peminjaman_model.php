...
class peminjaman_model extends ci_model{
    ...
    function data()
    {
        ...
        return $query = $this->db->get('perpusweb_peminjaman');
    }
    ...
    public function ambilId($table, $where)
   {
    ...
    public function buat_kode()   {
		  $this->db->select('RIGHT(perpusweb_peminjaman.id_pinjam,4) as kode', FALSE);
		  $this->db->order_by('id_pinjam','DESC');
		  $this->db->limit(1);
		  $query = $this->db->get('perpusweb_peminjaman');      //cek dulu apakah ada sudah ada kode di tabel.
		  if($query->num_rows() <> 0){
